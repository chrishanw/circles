...
def plot_four(
    ...
    gs = fig.add_gridspec(1, 1, height_ratios=[1], hspace=0.0)
    ...
    if level == "package" and package_top == "stacked":
        ...
        ax1.text(0., bottomA + 10, f"{timing_ms[0]} [ms/ev]", ha='center', va='bottom', fontsize=12)
        ax1.text(1., bottomB + 10, f"{timing_ms[1]} [ms/ev]", ha='center', va='bottom', fontsize=12)
        ax1.text(2., bottomC + 10, f"{timing_ms[2]} [ms/ev]", ha='center', va='bottom', fontsize=12)
        ax1.text(3., bottomD + 10, f"{timing_ms[3]} [ms/ev]", ha='center', va='bottom', fontsize=12)

        max_height = max(bottomA, bottomB, bottomC, bottomD)
        ax1.set_xlabel("HLT menu version", fontsize=20)
        ...
        ax1.legend(handles=pkg_handles, loc="center left", bbox_to_anchor=(1, 0.5), fontsize=15)
...
def main():
    ...
    p.add_argument("--label_fontsize", type=int, default=12, help="X tick font size")
    ...
    metric_label = "Average processing time per event [ms]"
    subtitle_bits = [
        f"level={args.level}",
